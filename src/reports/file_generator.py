...
details_columns = ["name", "age", "wage", "height", "season", "mins", "dist_90",
                   "position", "club", "nat", "personality", "weight", "transfer_value", "expires"]

offensive_columns = ["turn_diff", "poss_won_90", "poss_lost_90", "pr_passes_90", "pas_pct", "pas_a_90", "xa_90", "drb_90", "cr_c_90",
                     "crs_a_90", "crs_pct", "xpg_diff", "shot_pct", "shot_pct", "hdrs_w_90", "sprint_90", "off", "off_90", "np_xg_90", "gls_90", "xgp", "xgp_90"]

defensive_columns = ["defensive_score", "int_90", "poss_won_90",
                     "tck_90", "tck_r", "hdr_w_90", "hdr_pct", "clr_90", "blk_90"]

goalkeeping_columns = ["sv_pct", "svh", "svp", "svt", "goals_against",
                       "clean_sheets", "pen_saves", "pen_missed", "poss_lost_90", "pr_passes_90", "tck_r"]
...
def generate_html(player_compared: dict, matched_players: list[dict]) -> str:
    ...
    with open(
        f"output/report_season_{player_compared['season']
                                }_{player_compared['style']}_{player_compared['name']}.html",
        "w",
        encoding="utf-8"
    ) as f:
        f.write(html)
...
def generate_csv(player_compared: dict, matched_players: list[dict]) -> str:
    ...
    csv_path = (
        f"output/report_season_{player_compared['season']
                                }_{player_compared['style']}_{player_compared['name']}.csv"
    )
